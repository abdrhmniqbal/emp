...
import { $tracks, Track } from "@/modules/player/player.store";
import { useStore } from "@nanostores/react";
import {
    MAX_PLAYLIST_DESCRIPTION_LENGTH,
    MAX_PLAYLIST_NAME_LENGTH,
} from "@/modules/playlist/playlist.utils";
import { useCreatePlaylistScreen } from "../hooks/use-create-playlist";
    ...
    return (
        <View className="flex-1 bg-background">
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                ...
                >
                    <Text className="text-accent text-base font-medium">Add Tracks</Text>
                    <Ionicons
                ...
                {selectedTracksList.length > 0 && !showTrackPicker && (
                    <View className="mt-4">
                        <Text className="px-4 text-muted text-sm mb-2">
                            {selectedTracksList.length} {selectedTracksList.length === 1 ? 'track' : 'tracks'} selected
                        </Text>
