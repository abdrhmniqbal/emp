...
export default function PlaylistDetailsScreen() {
    ...
    const tracks = (playlist?.tracks || []).map((pt) => {
        ...
        return {
            ...t,
            artist: typeof t.artist === 'object' && t.artist ? (t.artist as any).name : t.artist,
            album: typeof t.album === 'object' && t.album ? (t.album as any).title : t.album,
            image: t.image || t.artwork || (typeof t.album === 'object' && t.album ? (t.album as any).artwork : undefined)
        } as unknown as Track;
    });

    let playlistImages: string[] = [];
    if (playlist?.artwork) {
        playlistImages = [playlist.artwork];
    } else {
        ...
        playlistImages = Array.from(images);
    }
            ...
            >
                ...
                <View className="px-6 pt-24 pb-6">
                    <View className="flex-row gap-4 pt-6">
                        {/* Playlist Cover - Collage or Single */}
                ...
                {/* Track List - No Disc Header, No Numbers */}
                <View className="px-4">
                    <SongList
                        data={tracks}
                        showNumbers={false}
